...
/*makes an aref for a multi dimensional array */
TOKEN makemultiaref(TOKEN var, TOKEN off, TOKEN tok);

/*reduces a var point */
TOKEN reducepoint(TOKEN var, TOKEN point);

/* takes a binop of a multiplication or division, and floats either parameter, if necessary */
...
/* Makes a NUMBERTOK with stringval val*/
TOKEN makestringtok(char* val);

/* Makes a NUMBERTOK with realval val*/
...
/* used to create a multi dimensional array by calling instarray sevarl times */
TOKEN instmultiarray(TOKEN bounds, TOKEN typetok);
...
/* makes a constant given the constant */
TOKEN makeuconstorvariable(TOKEN tok);

/* makes a constant given the constant */
TOKEN makeunsignedconst(TOKEN tok);
